...
public class InventoryState
{
    ...
    public void positionSlots()
    {
        ...
        {
            if (i != 0) offset = offset + 85;
            getSlotN(InventorySlotType.AMMO, i).setLocation(35 + offset, y + 50);
        }
    }

    public void render()
    {
        //PrimitiveRenderer.drawRectangle(new Rectangle(x, y, WIDTH, HEIGHT), new Color(70, 70, 70), 0);
        //for(SlotState s : slots) if(s.type != InventorySlotType.WEAPON) s.render();
    }

    public void renderHotbar()
    {
        PrimitiveRenderer.drawRectangle(new Rectangle(25, Resolution.HEIGHT - 120, 290, Resolution.HEIGHT - 25), new Color(69, 69, 42), 0.02f);
        for (SlotState s : slots) if (s.type == InventorySlotType.WEAPON) s.render();
    }
